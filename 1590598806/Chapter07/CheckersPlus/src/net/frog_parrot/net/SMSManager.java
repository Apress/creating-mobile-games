...
public class SMSManager {
   ...
   */
  private String myPortNum = "16474";
  ...
  public boolean checkPhoneNum(String phoneNumber) {
    //System.out.println("checkPhoneNum-->myPhoneNum: " + myPhoneNum);
    //System.out.println("checkPhoneNum-->phoneNumber: " + phoneNumber);
    if(myPhoneNum == null) {
      myPhoneNum = phoneNumber;
      return true;
    } else if (myPhoneNum.equals(phoneNumber)) {
      return true;
    } else {
      return false;
    }
  ...
  public void receiveInvitation(String taunt) {
    System.out.println("receiveInvitation-->taunt: " + taunt);
    int state = myManager.getState();
    if((state == MoveManager.NOT_STARTED) 
        || (state == MoveManager.FOUND_REMOTE_PLAYER)) {
      //System.out.println("got a new invitation");
      myManager.receiveInvitation(taunt, myPhoneNum.substring(6));
    }
  }
  ...
  public void sendInvitation(String phoneNumber) {
    myPhoneNum = SMS_PROTOCOL + phoneNumber;
    byte[] invitation = new byte[1];
    invitation[0] = START_GAME_FLAG;
    invitation = addTaunt(invitation);
    SMSSender sender = new SMSSender(myPhoneNum, myPortNum, 
        invitation, myManager);
    sender.start();
  }
  ...
  public void sendLocalMove() {
    //System.out.println("sendLocalMove-->move: " + myMove);
    if(myMove != null) {
      myMove = addTaunt(myMove);
      //System.out.println("sendLocalMove-->length: " + myMove.length);
      SMSSender sender = new SMSSender(myPhoneNum, myPortNum, 
          myMove, myManager);
      myMove = null;
      sender.start();
  ...
  public void setMove(byte[] move) {
    //System.out.println("SMSManager-->setMove");
    myManager.receiveRemoteMove(move);
  }

  /**
   * Pass the remote player's message to the display.
   */
  /*
  public void receiveTaunt(String taunt) {
    myManager.receiveTaunt(taunt);
  }
  */
