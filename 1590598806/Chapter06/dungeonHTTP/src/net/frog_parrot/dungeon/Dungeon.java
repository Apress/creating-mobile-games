...
public class Dungeon extends MIDlet implements CommandListener {
  ...
  /**
   * The command to download new boards.  
   */
  private Command myDownloadCommand 
    = new Command("Download boards", Command.SCREEN, 10);

  /**
   * The command to restore a previously saved game.  
   */
  private Command myRestoreCommand 
    = new Command("Restore Game", Command.SCREEN, 3);
  ...
  public void startApp() throws MIDletStateChangeException {
    try { 
      if(myCanvas == null) {
        ...
        myCanvas.addCommand(myNewCommand);
        myCanvas.addCommand(myDownloadCommand);
  ...
  public void commandAction(Command c, Displayable s) {
    try {
      if(c == myGoCommand) {
        ...
        destroyApp(false);
        notifyDestroyed();
      } else if(c == myDownloadCommand) {
        // spawn a new BoardReader thread to 
        // connect and download new boards:
        BoardReader br = new BoardReader(this, myCanvas);
        br.start();
