...
public class CheckersCanvas extends Canvas {
  ...
  /**
   * whether or not you're waiting for another player to join 
   * the game.
   */
  ...
  /**
   * Display a screen to prompt the player to invite 
   * another player.
   */
  void setInvitationScreen() {
    myIsWaiting = true;
  }

  /**
   * Display a screen to inform the player of an error.
   */
  void setErrorScreen(String errMsg) {
  }

  /**
   * Display a screen to tell the local player that 
   * the game is waiting for a response from the remote 
   * player. 
   */
  ...
  /**
   * This is called as soon as the game begins or when you 
  ...
  protected void paint(Graphics g) {
    ...
    // If you need to wait for another player to join the 
    // game before you can start, this displays the appropriate
    // message:
    if(myIsWaiting) {
    ...
    for(byte i = 0; i < 4; i++) {
      for(byte j = 0; j < 8; j++) {
        ...
        // now if this is a selected square, you draw it lighter
        // and outline it:
        if(myGame.isSelected(i, j)) {
          ...
          g.setColor(RED);
          g.drawRect((2*i + offset)*mySquareSize, j*mySquareSize, 
                       mySquareSize - 1, mySquareSize - 1);
        } else {
          // if it's not selected, you draw it dark grey:
          g.setColor(GREY);
          g.fillRect((2*i + offset)*mySquareSize, j*mySquareSize, 
                     mySquareSize, mySquareSize);
        }
    ...
    // you display the end message.
    if(myGame.getGameOver()) {
      //System.out.println("paint-->gameOver");
      // perform some calculations to place the text correctly:
      Font font = g.getFont();
      int fontHeight = font.getHeight();
