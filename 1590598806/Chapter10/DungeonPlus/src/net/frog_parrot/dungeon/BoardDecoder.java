...
public class BoardDecoder {
   ...
   * the data bytes.
   */
  public BoardDecoder(int boardNum, Customizer customizer) 
       throws Exception {
    // we start by selecting the two dimensional 
    // array corresponding to the desired board:
    byte[] data = null;
    if(boardNum < myData.length) {
      data = myData[boardNum];
    } else {
  ...
  /**
   * get the nubmer of hard-coded boards.
   */
  public static int getNumDefaultBoards() {
   ...
   */
  DoorKey[] createDoors(Image image) {
    DoorKey[] retArray = new DoorKey[8];
    for(int i = 0; i < 4; i++) {
      retArray[2*i] = new DoorKey(i, false, myDoors[2*i], image);
   ...
   * creating redundant sprites.)
   */
  DoorKey[] createKeys(Image image) {
    DoorKey[] retArray = new DoorKey[4];
    for(int i = 0; i < 4; i++) {
      retArray[i] = new DoorKey(i, true, myKeys[i], image);
    }
    return(retArray);
  }
  ...
  static void decodeDungeon(byte[] data, TiledLayer dungeon, 
    ...
    // to set the frame indices for each of the 16 rows.
    for(int i = 0; i < 16; i++) {
