...
public class DataConverter {
   ...
   */
  public static boolean checkEqual(byte[] byte1, byte[] byte2) {
  ...
  public static boolean checkIntConverter(int min, int max) {
    try {
      for(int i = min; i < max; i++) {
	if(! checkEqual(nostreamIntToFourBytes(i), intToFourBytes(i))) {
	  return false;
	}
      }
